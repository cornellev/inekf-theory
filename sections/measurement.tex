...
Left-Invariant measurement models observe sensors attached to the robot that measure in world coordinates.

\subsubsection{GPS Position Measurement}
We define the GPS position as the of the antenna in the ENU(East, North, Up) world frame. Let us define $\mathbf{l}$ as the vector going from the car axle to the GPS-antenna expressed relative to the body-frame. \textcolor{red}{(I believe the axle-to-GPS is the right way to define $\mathbf{l}$, but I could be wrong?)} \begin{equation}
\begin{gathered}
  \fv{W}{\dbf{p}}_{\mathrm{antenna}}=\fv{W}{\dbf{p}}_{\mathcal{B}}+\rotmat{W}{\mathcal{B}}\fv{\mathcal{B}}l
    \end{gathered}
\end{equation}
Defining our selection vector as $\mathbf{d}=\begin{bmatrix}
    1^\top,0,1
\end{bmatrix}^\top$:

\begin{equation}
    X\mathbf{d}=\begin{bmatrix}
      R & \mathbf{v} & \mathbf{p} \\ 
        0 & 1 & 0 \\ 
        0 & 0 & 1
    \end{bmatrix}
